...
func (s *Server) initI18n(engine *gin.Engine) error {
	...
	findI18nParamNames := func(key string) []string {
		...
		for i := 0; i < keyLen-1; i++ {
			if key[i:i+2] == "{{" { // check start with "{{"
				j := i + 2
				isFind := false
				for ; j < keyLen-1; j++ {
					if key[j:j+2] == "}}" { // end with "}}"
						isFind = true
						break
					}
				}
...
func (s *Server) startTask() {
	...
	go func() {
		...
		// every 10 seconds count traffic, delay 5 seconds for first time, avoid conflict with restart xray
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// every 30 seconds check inbound traffic limit and expired
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// every day notify traffic info, Shanghai time 8:30
...
func (s *Server) Start() (err error) {
	//this is anonymous function, no function name
	defer func() {
		if err != nil {
			s.Stop()
		}
